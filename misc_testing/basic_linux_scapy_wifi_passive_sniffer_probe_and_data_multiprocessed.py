# Basic script to scan for probe requests and activity in data frames, uses multiprocessing.
...
def catcher(pkt):
    """This function only puts the packet in the bucket and exits immediately."""
...
try:
    while True:
        try:
            sniff(iface=target_iface, prn=catcher, store=0, timeout=1, filter="type mgt subtype probe-req or type data") # START SNIFFING PACKETS
